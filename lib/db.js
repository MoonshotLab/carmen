...
const adapter = new FileSync('/tmp/stats.json');
const db = low(adapter);

const util = require('./util');

getFormattedStatsObj();
...
function getFormattedStatsObj() {
  ...
  const dbWeek = dbVal.weekly[weekNum];
  ...
  const dbMonth = dbVal.monthly[monthNum];
  dbMonth.rooms = util.getSortedRoomListByFrequencyRequested(dbMonth.rooms); // obj -> array

  return {
    week: Object.assign(dbWeek, { num: weekNum }),
    month: Object.assign(dbMonth, { name: monthName }),
    total: dbVal.total
  };
}
