...
const roomList = rooms.map(roomObj => _.camelCase(roomObj.name));
...
const getRoomFromRoomList = searchRoom => {
  if (!!searchRoom && searchRoom.length > 0) {
    let lowerSearchRoom = searchRoom.toLowerCase();

    if (lowerSearchRoom === 'albert 2') lowerSearchRoom = 'albert ii';
