...
function getUtteranceByType(type) {
  switch (type) {
    ...
    case 'newGreeting':
      return randomPick(utterances.newGreeting);
    case 'feedback':
      return randomPick(utterances.feedback);
    case 'thanks':
...
function logAttachmentSent(url, to) {
  log(`Sending attachment '${url}' to ${to}`);
}
...
function replyToMessageWithAttachment(bot, message, attachmentUrl) {
  logMessageSent(attachmentUrl, message.from);
...
function followUpIfNecessary(bot, message, roomObj) {
  if ('img' in roomObj) {
    ...
    delay(() => bot.reply(message, followUpCopy), config.followUpTimeout);
  }
}
...
exports.replyToMessageWithAttachment = replyToMessageWithAttachment;
exports.delay = delay;
