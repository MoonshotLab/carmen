...
const roomList = rooms.map(roomObj => {
  ...
  return {
    name: roomName,
    pattern: new RegExp(`(^|[^a-zA-Z])${roomName}(?![a-zA-Z])`, 'i') // only match a string if it's not surrounded by other letters (so 'hr' doesn't match 'bathroom' 😅)
  };
});
...
const getSortedRoomListByFrequencyRequested = roomsObj => {
  // turn obj into array
  const roomList = [];
...
const getRoomFromRoomList = searchRoom => {
  if (!!searchRoom && searchRoom.length > 0) {
    ...
    if (camelSearchRoom === 'albert 2') camelSearchRoom = 'albert ii';

    for (let i = 0; i < rooms.length; i++) {
      let room = rooms[i];
      if (camelSearchRoom === _.camelCase(room.name)) return room;
    }
  }
