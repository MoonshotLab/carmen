...
// only match a string if it's not surrounded by other letters (so 'hr' doesn't match 'bathroom' 😅)
const roomListRegexes = rooms.map(
  roomObj => new RegExp(`(^|[^a-zA-Z])${roomObj.name}(?![a-zA-Z])`, 'i')
);
...
function handleRoomQuery(bot, message) {
  util
